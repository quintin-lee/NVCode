...
| 步骤 | 快捷键 | 说明 |
|------|--------|------|
| 1 | `<leader>gc` | 选择 gitmoji 和 scope |
| 2 | 在浮动窗口中编写提交信息 |
| 3 | `<C-s>` 确认提交 / `<C-c>` 取消 |
...
| 步骤 | 快捷键 | 说明 |
|------|--------|------|
| 1 | `<leader>gd` | 对比当前文件与 index |
| 2 | `]h` / `[h` | 在 hunks 之间导航 |
| 3 | `<leader>hp` | 预览 hunk |
| 4 | `<leader>hs` / `<leader>hr` | 暂存或重置 hunk |
| 5 | `<leader>xx` | 打开 Trouble 诊断面板 |
...
|------|--------|------|
| 1 | `<leader>oo` | 打开任务面板 |
| 2 | `<leader>or` | 列出可用任务（自动从 LSP 发现） |
| 3 | 选择任务 | 在底部面板中运行 |
| 4 | `<leader>ob` | 快速重新运行上次构建 |
...
| 1 | `]e` / `[e` | 跳转错误 |
| 2 | `]w` / `[w` | 跳转警告 |
| 3 | `<leader>cd` | 当前行诊断浮动窗口 |
| 4 | `<leader>xx` | 打开 Trouble 诊断列表 |
| 5 | `<leader>ud` | 切换诊断显示 |
